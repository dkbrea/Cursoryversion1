...
export function SavingsGoalsCard({ refreshTrigger, isMobile = false }: SavingsGoalsCardProps = {}) {
  ...
  return (
    <Card>
      <CardHeader className={`flex flex-row items-center justify-between space-y-0 ${isMobile ? 'pb-3' : 'pb-2'}`}>
        <CardTitle className={`${isMobile ? 'text-base' : 'text-sm'} font-medium`}>Savings Goals</CardTitle>
        <Target className={`${isMobile ? 'h-5 w-5' : 'h-4 w-4'} text-muted-foreground`} />
      </CardHeader>
      <CardContent>
        <div className={`${isMobile ? 'space-y-3' : 'space-y-4'}`}>
          {topGoals.slice(0, isMobile ? 2 : 3).map((goal) => (
            <div key={goal.id} className={`${isMobile ? 'space-y-2' : 'space-y-2'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className={`${isMobile ? 'w-3 h-3' : 'w-2 h-2'} rounded-full bg-blue-500`} />
                  <span className={`${isMobile ? 'text-sm' : 'text-sm'} font-medium truncate`}>{goal.name}</span>
                </div>
                <span className={`${isMobile ? 'text-sm' : 'text-sm'} font-medium`}>{Math.round(goal.progress)}%</span>
              </div>
              
              <Progress value={goal.progress} className={`${isMobile ? 'h-3' : 'h-2'}`} />
              ...
              <div className={`flex ${isMobile ? 'flex-col space-y-1' : 'items-center justify-between'} ${isMobile ? 'text-sm' : 'text-xs'} text-muted-foreground`}>
                <div className="flex items-center space-x-1">
                  <DollarSign className={`${isMobile ? 'h-4 w-4' : 'h-3 w-3'}`} />
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className={goal.isOverdue ? "text-red-500" : ""}>
                    Target: {format(new Date(goal.targetDate), isMobile ? "MMM d, yyyy" : "MMM d, yyyy")}
                  </span>
                </div>
              </div>
            </div>
          ...
          {goals.length > (isMobile ? 2 : 3) && (
            <div className="text-center pt-2">
              <p className={`${isMobile ? 'text-sm' : 'text-xs'} text-muted-foreground`}>
                Showing {isMobile ? 2 : 3} of {goals.filter(g => g.currentAmount < g.targetAmount).length} active goals
              </p>
            </div>
          )}
        </div>
