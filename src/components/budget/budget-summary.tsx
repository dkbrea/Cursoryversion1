...
export function BudgetSummary({
  ...
  const balancedMessages = [
    "Your budget isn’t a snapshot — it’s a strategy. You’ve assigned every dollar. Now track your plan in action.",
    "Every dollar is deployed — now track how your plan performs.",
    "You’re the strategist. This is your playbook. Make every move count.",
    "Your budget is a working document. Monitor it. Adjust it. Own it."
  ];
  // Pick a random message on each render
  const randomBalancedMessage = balancedMessages[Math.floor(Math.random() * balancedMessages.length)];
  ...
  const getLeftToAllocateStatus = (): StatusType => {
    if (isBalanced) {
      return {
        message: randomBalancedMessage,
