...
import type { PaycheckPreferences, PaycheckAllocationMode, PaycheckManualPlan } from '@/types';
...
export const getUserPreferences = async (userId: string): Promise<{ preferences: UserPreferences | null; error?: string }> => {
  try {
    ...
    // --- PATCH: Ensure manualPlanDateRanges is always present and parsed ---
    let paycheckPreferences = data.paycheck_preferences || {};
    if (paycheckPreferences && typeof paycheckPreferences === 'string') {
      try { paycheckPreferences = JSON.parse(paycheckPreferences); } catch {}
    }
    if (!paycheckPreferences.manualPlanDateRanges) {
      paycheckPreferences.manualPlanDateRanges = {};
    }
...
export const createDefaultUserPreferences = async (userId: string): Promise<{ preferences: UserPreferences | null; error?: string }> => {
  try {
    ...
    const defaultPaycheckPreferences: PaycheckPreferences = {
      ...
      allocationMode: 'auto', // Default to auto
      activeManualPlan: null,  // No plan active by default
      manualPlanDateRanges: {} // PATCH: always present
    };
...
export const updateUserPreferences = async (
  ...
  try {
    ...
    if (updates.paycheckPreferences !== undefined) {
      // PATCH: Ensure manualPlanDateRanges is always present
      let prefs = updates.paycheckPreferences;
      if (prefs && typeof prefs === 'string') {
        try { prefs = JSON.parse(prefs); } catch {}
      }
      if (!prefs.manualPlanDateRanges) {
        prefs.manualPlanDateRanges = {};
      }
      updateData.paycheck_preferences = prefs;
    }
